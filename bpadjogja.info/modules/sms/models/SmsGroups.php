...
class SmsGroups extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('group_name, group_desc, creation_date, creation_id, modified_id', 'required'),
			...
			array('group_id, status, group_name, group_desc, creation_date, creation_id, modified_date, modified_id', 'safe', 'on'=>'search'),
		);
	}
		...
		if(count($this->defaultColumns) == 0) {
			...
			*/
			...
			if(!isset($_GET['type'])) {
				$this->defaultColumns[] = array(
					'name' => 'status',
					'value' => 'Utility::getPublish(Yii::app()->controller->createUrl("status",array("id"=>$data->group_id)), $data->status, 1)',
					'htmlOptions' => array(
						'class' => 'center',
					),
					'filter'=>array(
