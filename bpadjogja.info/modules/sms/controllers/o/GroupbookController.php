...
class GroupbookController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = SmsGroupPhonebook::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('SmsGroupPhonebook', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = 'Sms Group Phonebooks';
	...
	public function actionView($id) 
	{
		...
		$this->pageTitle = 'View Sms Group Phonebooks';
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['SmsGroupPhonebook'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-sms-group-phonebook',
							'msg' => '<div class="errorSummary success"><strong>SmsGroupPhonebook success updated.</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', 'SmsGroupPhonebook success updated.');
					//$this->redirect(array('view','id'=>$model->id));
					$this->redirect(array('manage'));
				}
			}
		...
		$this->pageTitle = 'Update Sms Group Phonebooks';
		$this->pageDescription = '';
		$this->pageMeta = '';
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				SmsGroupPhonebook::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				SmsGroupPhonebook::model()->deleteAll($criteria);
			}
		}
			...
			if(isset($id)) {
				...
				if($model->update()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-sms-group-phonebook',
						'msg' => '<div class="errorSummary success"><strong>SmsGroupPhonebook success published.</strong></div>',
					));
				}
			}
