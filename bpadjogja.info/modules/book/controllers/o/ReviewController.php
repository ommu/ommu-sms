...
class ReviewController extends Controller
{
	...
	{
		$model=new BookReviews('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['BookReviews'])) {
			$model->attributes=$_GET['BookReviews'];
		}
		...
		$this->pageTitle = 'Book Reviews Manage';
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['BookReviews'])) {
			$model->attributes=$_POST['BookReviews'];
			...
			if(strlen($jsonError) > 2 || strlen($jsonErrorBook) > 2 || strlen($jsonErrorAuthor) > 2) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					...
					if($authorModel != null) {
						$model->resensator_id = $authorModel->author_id;
					} else {
						if($author->save())
							$model->resensator_id = $author->author_id;
					}
						...
						if($bookModel != null) {
							...
							if($model->save()) {
								echo CJSON::encode(array(
									...
									'id' => 'partial-book-reviews',
									'msg' => '<div class="errorSummary success"><strong>BookReviews success created.</strong></div>',
								));
							} else {
			...
			$this->pageTitle = 'Create Book Reviews';
			$this->pageDescription = '';
			$this->pageMeta = '';
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['BookReviews'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-book-reviews',
							'msg' => '<div class="errorSummary success"><strong>BookReviews success updated.</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			$this->pageTitle = 'Update Book Reviews';
			$this->pageDescription = '';
			$this->pageMeta = '';
	...
	public function actionView($id) 
	{
		...
		$this->pageTitle = 'View Book Reviews';
		$this->pageDescription = '';
		$this->pageMeta = '';
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				BookReviews::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				BookReviews::model()->deleteAll($criteria);
			}
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				if($model->delete()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-book-reviews',
						'msg' => '<div class="errorSummary success"><strong>BookReviews success deleted.</strong></div>',
					));
				}
			}
			...
			$this->pageTitle = 'BookReviews Delete.';
			$this->pageDescription = '';
			$this->pageMeta = '';
			...
			if(isset($id)) {
				...
				if($model->update()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-book-reviews',
						'msg' => '<div class="errorSummary success"><strong>BookReviews success published.</strong></div>',
					));
				}
			}
